...
--! ProjectFramework.Entities\n
--! See Entities for the module on entities.\n\n
--! ProjectFramework.Entities.List\n
--! A list of all non-instanced entities in the game.

ProjectFramework = ProjectFramework or {}

--! \brief Entities module for ProjectFramework. Defines and interacts with ENTITY object.
--! \class Entities
ProjectFramework.Entities = {}
ProjectFramework.Entities.__index = ProjectFramework.Entities
ProjectFramework.Entities.List = {}
ProjectFramework.Entities = ProjectFramework.Foundation:NewModule(ProjectFramework.Entities, "Entities")

--! \brief Entity class for ProjectFramework.
--! \class ENTITY
local ENTITY = {}
...
function ENTITY:Initialize()
    ...
    return ProjectFramework.Entities:Initialize(self, self.name)
end
...
function ProjectFramework.Entities:New(name)
    local object = {
        name = name,
...
function ProjectFramework.Entities:Initialize(data, name)
    ProjectFramework.Entities.List[name] = data
...
function ProjectFramework.Entities:GetEntityByID(entityID)
    local entity = ProjectFramework.Entities.List[entityID] or nil
   
    return entity
end

function ProjectFramework.Entities:GetData(worldObject, index)
    if worldObject then
...
function ProjectFramework.Entities:SetData(worldObject, index, value)
...
function ProjectFramework.Entities:IsEntity(object)
    for id, entity in pairs(ProjectFramework.Entities.List) do
...
function ProjectFramework.Entities:EmitSound(worldObject, sound)
...
function ProjectFramework.Entities.OnObjectAdded(object)
    local isEntity, entityID = ProjectFramework.Entities:IsEntity(object)
    
    if isEntity then
        local entity = ProjectFramework.Entities:GetEntityByID(entityID)
...
Events.OnObjectAdded.Add(ProjectFramework.Entities.OnObjectAdded)

function ProjectFramework.Entities.OnObjectAboutToBeRemoved(object)
	local isEntity, entityID = ProjectFramework.Entities:IsEntity(object)
...
Events.OnObjectAboutToBeRemoved.Add(ProjectFramework.Entities.OnObjectAboutToBeRemoved)

function ProjectFramework.Entities.OnPreFillWorldObjectContextMenu(player, context, worldObjects, test)
...
function ProjectFramework.Entities.OnGameStart()
    Events.OnPreFillWorldObjectContextMenu.Add(ProjectFramework.Entities.OnPreFillWorldObjectContextMenu)
end

function ProjectFramework.Entities:LoadGridsquare(square)
